...
export async function POST(request: Request) {
  ...
  const outcome = await generateJson({
    ...
    userPrompt: `${teacherPersona()} ${groundingRules()} Konu: ${parsedBody.data.topic}. Gerçek bir sözlü sınav gibi 5 soru yaz. Sorular belgedeki kavramı yoklasın; cevap belgede yoksa o soruyu sorma.`,
    parse: (raw) => {
