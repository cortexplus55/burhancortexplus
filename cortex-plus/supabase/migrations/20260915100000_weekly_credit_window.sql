-- Haftalık abonede kota penceresi gerçekten 7 gün olsun.
--
-- İki hata vardı ve ikisi de göç dosyaları YEREL BİR POSTGRES'TE
-- ÇALIŞTIRILANA kadar görünmedi: birim testleri SQL metnini okuyor, şemayı
-- kurmuyor.
--
-- 1) `finalize_paytr_payment` cüzdan penceresini ödeme anında
--    `period_kind = 'monthly'` ve `+ 30 days` olarak SABİT yazıyordu.
--    20260914130000 penceresi plandan türetmeyi `credit_reserve`e eklemişti
--    ama orası yalnızca pencere DOLDUĞUNDA çalışıyor — ödeme anında 30 gün
--    damgalandığı için o dal ilk 30 gün boyunca hiç çalışmıyordu.
--
--    Ölçülen sonuç: haftalık abonelikte abonelik süresi 7 gün çıkıyor
--    (doğru) ama kredi penceresi 30 gün (yanlış). Öğrenci 349 TL'ye aldığı
--    150 krediyi bir ay boyunca harcayabiliyordu.
--
-- 2) `credit_reserve` yenileme dalı `period_kind = 'weekly'` yazıyor ama
--    `credit_wallets_period_kind_check` yalnızca 'daily' ve 'monthly'
--    kabul ediyordu. Yani o dal ilk çalıştığında kısıt ihlaliyle patlardı —
--    öğrencinin sohbeti 500 ile düşerdi.

ALTER TABLE public.credit_wallets
  DROP CONSTRAINT IF EXISTS credit_wallets_period_kind_check;
ALTER TABLE public.credit_wallets
  ADD CONSTRAINT credit_wallets_period_kind_check
  CHECK (period_kind IN ('daily', 'weekly', 'monthly'));

COMMENT ON COLUMN public.credit_wallets.period_kind IS
  'Kota penceresinin cinsi: daily (ücretsiz), weekly (haftalık paket), monthly';

CREATE OR REPLACE FUNCTION public.finalize_paytr_payment(p_merchant_oid text, p_payload_hash text, p_status text, p_raw_payload jsonb DEFAULT '{}'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
  ...
  IF v_is_subscription THEN
    ...
    UPDATE public.credit_wallets
       SET free_allowance_remaining = GREATEST(
           ...
           period_kind = CASE WHEN v_billing_period = 'weekly'
                              THEN 'weekly' ELSE 'monthly' END,
           period_ends_at = date_trunc('day', now())
             + make_interval(days => CASE WHEN v_billing_period = 'weekly'
                                          THEN 7 ELSE 30 END),
           updated_at = now()
     WHERE user_id = v_beneficiary;
  END IF;
...
-- Yetkiler açıkça yeniden yazılıyor. `CREATE OR REPLACE` var olan hakları
-- koruyor, ama bu satırlar bir kez sessizce düşürülmüştü ve fonksiyon
-- herkese açık kalmıştı; açıkça yazmak o hatayı imkânsız kılıyor.
