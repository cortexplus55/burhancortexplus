-- Veli kaydı: yakınlık + isteğe bağlı telefon. Veli onboarding'i çocuk bağlantısına kadar açık kalır.

ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS phone text,
  ADD COLUMN IF NOT EXISTS parent_relation text;

ALTER TABLE public.profiles DROP CONSTRAINT IF EXISTS profiles_parent_relation_check;
ALTER TABLE public.profiles
  ADD CONSTRAINT profiles_parent_relation_check
  CHECK (
    parent_relation IS NULL
    OR parent_relation IN ('anne', 'baba', 'vasi', 'diger')
  );
...
BEGIN
  ...
  VALUES (
    ...
    CASE
      ...
      WHEN v_meta->>'onboarding_done' = 'true' THEN now()
      ELSE NULL
    END,
